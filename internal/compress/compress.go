...
func (s *Service) CompressColumn(block *col.ColumnBlock) error {
	...
	metaValue, err := s.meta.Lookup(block.Table)
	if err != nil {
		return err
	}
	...
	if err != nil {
		return fmt.Errorf("compress column %s: %w", block.Column, err)
	}
	dict := codec.dict
	...
	if err := s.meta.MarkCompressedAtVersion(block.Table, metaValue.SchemaVersion.Number); err != nil {
		return err
	}
	return nil
}
