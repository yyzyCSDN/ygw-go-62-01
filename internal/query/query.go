...
type Engine struct {
	...
	chunk   int
	cache   map[string]*meta.ColumnMeta
}
...
func NewEngine(store *col.Store, parts *part.Registry, metaRegistry *meta.Registry, dicts *compress.DictRegistry) *Engine {
	return &Engine{
		...
		cache:   make(map[string]*meta.ColumnMeta),
	}
}
...
func (e *Engine) Build(table string, from, to time.Time, columns []string, pred *scan.Predicate) (*Plan, error) {
	metaValue, ok := e.cache[table]
	if !ok {
		current, err := e.catalog.MetaFor(table)
		if err != nil {
			return nil, err
		}
		metaValue = current
		e.cache[table] = metaValue
	}
...
func (e *Engine) blocksFor(plan *Plan, p *part.Partition) []uint64 {
	pos := plan.Meta.Schema.Position(first(plan.Columns))
	if pos < 0 || pos >= len(p.BlockIDs) {
		return nil
	}
	return []uint64{p.BlockIDs[pos]}
}
