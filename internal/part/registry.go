...
// RemoveWithBlocks drops a partition and returns the block ids it held.
func (r *Registry) RemoveWithBlocks(id uint64) ([]uint64, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	p, ok := r.byID[id]
	if !ok {
	...
	return append([]uint64(nil), p.BlockIDs...), nil
}
