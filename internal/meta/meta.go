...
// VerifyVersion confirms the latest metadata of a table still matches the
// version a query plan was built against.
func (r *Registry) VerifyVersion(table string, number uint64) error {
	_, err := r.LookupAtVersion(table, number)
	return err
}
